...
  $('#view-gate').classList.toggle('hidden', !show);
  // hide all other views if showing gate
  ['list','submit-level','submit-record','rankings','guidelines','admin'].forEach(v => {
    document.getElementById('view-'+v).classList.toggle('hidden', show);
  });
}
...
  if (!u) {
    currentUser = null;
    $('#userArea').innerHTML = html`<button id="signin" class="px-3 py-1.5 rounded-lg bg-indigo-600 text-white">Sign in</button>`;
...
  let role = 'user', banned = false, displayName = u.displayName || 'User';
  if (usrSnap.exists()) {
    const d = usrSnap.data();
    role = d.role || 'user'; banned = !!d.banned; displayName = d.displayName || displayName;
  }
...
  $('#nav-admin').classList.toggle('hidden', !hasRole(currentUser,'mod'));
  showGate(false);
  renderAll();
});
...
function navTo(id) {
  for (const r of routes) $('#view-'+r).classList.add('hidden');
...
$('#nav-list').onclick = () => navTo('list');
$('#nav-submit-level').onclick = () => navTo('submit-level');
$('#nav-submit-record').onclick = () => navTo('submit-record');
$('#nav-rankings').onclick = () => { navTo('rankings'); renderRankings(); };
$('#nav-guidelines').onclick = () => navTo('guidelines');
$('#nav-admin').onclick = () => { navTo('admin'); renderAdmin(); };
...
  $('#addLevelBtn').classList.toggle('hidden', !hasRole(currentUser,'mod'));
}
$('#addLevelBtn')?.addEventListener('click', () => $('#addLevelModal').classList.remove('hidden'));
$('#closeAddLevel')?.addEventListener('click', () => $('#addLevelModal').classList.add('hidden'));
...
  if (!name || !gdId) { toast('Name & GD ID required', false); return; }
  const qLast = query(colLevels(), orderBy('rank','desc'), limit(1));
  const lastSnap = await getDocs(qLast);
  let nextRank = 1; lastSnap.forEach(d => nextRank = (d.data().rank || 0) + 1);
...
  $('#lvl_name').value=''; $('#lvl_gdid').value=''; $('#lvl_desc').value=''; $('#lvl_verif').value='';
  renderList();
});

// Submissions
$('#submitLevelBtn')?.addEventListener('click', async () => {
  if (!currentUser) { toast('Sign in to submit', false); return; }
...
  if (!name || !gdId || !verificationVideo) { toast('All fields required', false); return; }
...
  $('#sub_name').value=''; $('#sub_gdid').value=''; $('#sub_desc').value=''; $('#sub_verif').value='';
});
...
  if (!levelName || !video || isNaN(progress)) { toast('All fields required', false); return; }
  if (progress < 60) { toast('Progress must be 60% or higher', false); return; }
  await addDoc(colSubmits(), {
    type:'record',
...
  $('#rec_levelname').value=''; $('#rec_progress').value=''; $('#rec_video').value='';
});

// Admin
async function renderAdmin() {
  if (!hasRole(currentUser,'mod')) { $('#view-admin').innerHTML = '<div class="p-4">No access.</div>'; return; }

  // Pending submissions
  const qSubs = query(colSubmits(), where('status','==','pending'), orderBy('createdAt','asc'));
  const subsSnap = await getDocs(qSubs);
  const subs = []; subsSnap.forEach(s => subs.push({ id: s.id, ...s.data() }));
...
  const users = []; uSnap.forEach(u => users.push({ id:u.id, ...u.data() }));
...
      <div>#</div><div>Player</div><div>Points</div>
    </div>
    ${rows.map((r, i)=> html`
